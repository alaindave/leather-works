...
const useTaskStore = create((set, get) => ({
    ...
    loadTopTasks: async (userId) => {
        ...
        try {
            ...
            console.error("An error occured while loading top tasks in Zustand", error);
            set({ loading: false });
        }
    },
    createTask: async (taskData) => {
        ...
        try {
            ...
            set((state) => ({
                tasks: state.tasks.map((t) => t._id === optimisticTask._id ? savedTask : t),
            }));
        }
        catch (error) {
            set((state) => ({
            ...
            console.error("An error occured while creating the task in Zustand", error);
        }
    },
    updateTask: async (updatedTask) => {
        ...
        try {
            ...
            set({ tasks: previous });
            console.error("An error occured while updating the task in Zustand", error);
        }
    },
    ...
    addComment: async (taskId, author, comment) => {
        ...
        set((state) => ({
            tasks: state.tasks.map((task) => task._id !== taskId
                ? task
                : {
                    ...task,
        ...
        try {
            ...
            set((state) => ({
                tasks: state.tasks.map((task) => task._id !== taskId
                    ...
                    : {
                        ...task,
                        comments: task.comments?.map((c) => c._id === tempId ? savedComment : c),
                    }),
            }));
        }
        catch (error) {
